...
class GamePersistenceIT extends GameAcceptanceSupport {
    @Test void globalHistoryIncludesThreeUsersAndStablePagination() {
        List<UUID> roundIds = new ArrayList<>(); List<UUID> userIds = new ArrayList<>();
        for (int i = 0; i < 3; i++) {
            Player user = user("History" + i, 1000); userIds.add(user.id());
        ...
        assertThat(all.items()).extracting(History::userId).containsAll(userIds);
