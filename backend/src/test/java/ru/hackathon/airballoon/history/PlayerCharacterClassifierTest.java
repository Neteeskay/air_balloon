...
class PlayerCharacterClassifierTest {
    ...
    @Test void cautious() { assertCode(round("1.40", "4.00", false), PlayerCharacterClassifier.Code.CAUTIOUS); }
    @Test void coldBlooded() { assertCode(round("5.00", "8.00", false), PlayerCharacterClassifier.Code.COLD_BLOODED); }
    @Test void closeCall() { assertCode(round("5.90", "6.00", false), PlayerCharacterClassifier.Code.CLOSE_CALL); }
    @Test void boosterHunter() { assertCode(round("2.50", "4.00", true), PlayerCharacterClassifier.Code.BOOSTER_HUNTER); }
    @Test void greedy() { assertCode(round(null, "2.01", false), PlayerCharacterClassifier.Code.GREEDY); }
    @Test void fallbackAdventurer() { assertCode(round(null, "1.80", false), PlayerCharacterClassifier.Code.ADVENTURER); }
    ...
    private static GameRound round(String cashout, String crash, boolean boosterActivated) {
        ...
        BigDecimal cashoutMultiplier = cashout == null ? null : new BigDecimal(cashout);
        return new GameRound(UUID.randomUUID(), UUID.randomUUID(), GameRound.Theme.GREEN, 100,
                ...
                new BigDecimal(crash), cashoutMultiplier, cashoutMultiplier == null ? 0 : 100, 0,
