...
Авторизованный пользователь может открыть раздел «Проверка» в верхней навигации,
вставить JSON из кнопки «Скопировать proof» или выбрать локальный `.json` файл.
Проверка SHA-256 и расчёт `X_final` выполняются в браузере; файл и proof не
отправляются на сервер. Результат отдельно показывает совпадение commitment,
фактический crash и рассчитанный X.
