...
export default function LoginPage({
  ...
  onAuthenticated: (login: string, password: string) => Promise<unknown>
  onBack: () => void
}) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage('');
    try {
      await onAuthenticated(login, password);
      setSuccess(true); setMessage('Добро пожаловать!');
    } catch (error) { setSuccess(false); setMessage(error instanceof Error ? error.message : 'Не удалось войти'); }
    setBusy(false);
  };
