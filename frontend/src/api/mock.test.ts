...
describe('deterministic mock adapters', () => {
  ...
  it('creates a WIN-only Scenario 8 offer and credits tickets idempotently', async () => {
    const r = await backend.api.game.startRound({ theme: 'GREEN', betAmount: 100, boosterMultiplier: 1 })
    now += 2000; backend.tick(); await backend.api.game.cashout(r.id, 'cashout-key'); now += 20_000; backend.tick()
    const offer = await backend.api.upsell.getOffer(r.id)
    expect(offer).toEqual(expect.objectContaining({ status: 'AVAILABLE', price: 150, ticketCount: 3 }))
    const first = await backend.api.upsell.purchase(offer!.offerId, 'purchase-key')
    const replay = await backend.api.upsell.purchase(offer!.offerId, 'purchase-key')
    expect(first).toMatchObject({ price: 150, ticketCount: 3, lotteryTicketCount: 3 })
    expect(replay).toMatchObject({ ...first, replayed: true })
    expect((await backend.api.economy.getBalance()).lotteryTicketCount).toBe(3)
  })
  it('never creates a Scenario 8 offer for LOSS', async () => {
    backend.api.dev!.setPreset('LOSE')
    const r = await backend.api.game.startRound({ theme: 'GREEN', betAmount: 100, boosterMultiplier: 1 })
    now += 8100; backend.tick()
    expect(await backend.api.upsell.getOffer(r.id)).toBeNull()
  })
})
