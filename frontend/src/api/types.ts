...
export type Catalog = { stakes: number[]; boosters: number[]; thresholds: Record<Theme, number[]>; pointsPerLevel: number; cashoutPoints: number }
export type Fairness = { roundId: string; commitment: string; status: 'COMMITTED' | 'REVEALED'; serverSeed?: string; crashMultiplier?: number; boosterLevel?: number; verified?: boolean; canonicalInput?: string; example?: boolean }
export type Round = {
...
export type HistoryItem = { roundId: string; username: string; theme: Theme; betAmount: number; boosterMultiplier: number; cashoutMultiplier?: number; crashMultiplier: number; winAmount: number; roundScore: number; result: 'WIN' | 'LOSS'; finishedAt: string }
export type HistoryPage = { items: HistoryItem[]; page: number; size: number; total: number }
...
export interface Api {
  mode: 'mock' | 'real'
  auth: { demos: (User & { password: string })[]; currentUser(): Promise<User | null>; login(login: string, password: string): Promise<User>; logout(): Promise<void> }
