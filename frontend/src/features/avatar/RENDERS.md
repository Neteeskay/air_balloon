...
В интерфейсе доступны два головных убора (Авиатор, Соломенная шляпа) и два аксессуара на шею (Красная бабочка, Облачный шарфик). Облачный шарфик открывается после завершения пазла «Небесное путешествие». Снятие аксессуаров отключено: всегда выбирается одна из четырёх пар.
...
Состояние образа валидируется mock-адаптером: заблокированный или неизвестный предмет нельзя надеть. Имя питомца, разблокированные предметы и текущий образ входят в единое состояние пользователя в `sessionStorage`.
...
Тестовый вход → кнопка профиля → «Открыть гардероб».
