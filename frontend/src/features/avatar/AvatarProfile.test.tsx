...
describe('profile wardrobe', () => {
  ...
  it('shows a locked item but cannot equip it', () => {
    ...
    expect(screen.getByText('Даёт буст ×2 один раз.')).toBeInTheDocument()
    expect(screen.getByText('✦ Буст ×2 один раз')).toBeInTheDocument()
    expect(screen.queryByText('✦ Меняет внешний вид')).not.toBeInTheDocument()
  ...
  it('opens the full puzzle collection from the profile preview', () => {
    ...
    expect(within(collection).getAllByText('🔒 Скоро')).toHaveLength(2)
    expect(within(collection).getAllByLabelText('Пазл пока недоступен')).toHaveLength(2)
  })
})
