...
import { clearPersistedRound, readPersistedRound } from './useRoundPersistence'
...
export function useGameSession() {
  ...
  useEffect(() => {
    ...
    api.auth.currentUser().then(async current => {
      ...
      if (current) {
        try {
          const persisted = readPersistedRound(`user:${current.id}`)
          const [wallet, catalog, profileValue] = await Promise.all([
            api.economy.getBalance(),
            api.catalog.get(),
            api.profile.get().catch(() => null),
          ])
          // The server remains authoritative. The local snapshot is only a
          // one-hour fallback for a reload while the connection is unavailable.
          let active = null as Awaited<ReturnType<typeof api.game.getActiveRound>>
          let activeLoadFailed = false
          try {
            active = await api.game.getActiveRound()
          } catch {
            activeLoadFailed = true
          }
          const restored = activeLoadFailed ? persisted : active
          if (!live) return
          setSession(s => ({ ...s, balance: wallet.bonusBalance, round: restored ? { bet: Number(restored.betAmount), boosterMultiplier: restored.boosterMultiplier as any, roundId: restored.id, showCashoutHint: false } : null, theme: restored ? restored.theme.toLowerCase() as any : s.theme }))
  ...
  const login = useCallback(async (loginName: string, password: string) => {
    const current = await api.auth.login(loginName, password)
    setUser(current)
    const persisted = readPersistedRound(`user:${current.id}`)
    const [wallet, catalog, profileValue] = await Promise.all([api.economy.getBalance(), api.catalog.get(), api.profile.get().catch(() => null)])
    const restored = await api.game.getActiveRound().catch(() => persisted)
    setSession(s => ({
      ...s,
      balance: wallet.bonusBalance,
      round: restored ? { bet: Number(restored.betAmount), boosterMultiplier: restored.boosterMultiplier as 1|2|3|4, roundId: restored.id, showCashoutHint: false } : null,
      theme: restored ? restored.theme.toLowerCase() as 'green' | 'red' : s.theme,
    }))
    setBetOptions(catalog.stakeOptions.map((o, i) => ({ id: i + 1, cost: Number(o.amount), multiplier: o.boosterMultiplier as 1|2|3|4 })))
    setProfile(profileValue)
    setError('')
  ...
  const logout = useCallback(async () => { if (user) clearPersistedRound(`user:${user.id}`); await api.auth.logout(); setUser(null); setSession(initialSession()) }, [user])
  ...
  const finishRound = useCallback(async (): Promise<Result | null> => { const id = session.round?.roundId; let resolved: Result | null = null; if (id) { try { resolved = await api.game.getResult(id); setResult(resolved) } catch { /* result view can be retried by the next refresh */ } } if (user) clearPersistedRound(`user:${user.id}`); setSession(s => ({ ...s, round: null })); await refresh(); return resolved }, [refresh, session.round?.roundId, user])
