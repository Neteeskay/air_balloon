...
export function useGameSession() {
  ...
  return { ...session, user, loading, error, result, profile, betOptions, api, login, logout, finishRound, startRound, repeatBet, equip, clearResult: () => setResult(null), switchTheme: () => setSession(s => ({ ...s, theme: s.theme === 'green' ? 'red' : 'green' })), toggleSound: () => setSession(s => ({ ...s, soundOn: !s.soundOn })), topUpBalance: () => {}, clearError: () => setError('') }
}
