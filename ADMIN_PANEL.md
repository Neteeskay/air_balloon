...
### Понятный интерфейс (UI-доработка)

- **Всё по-русски**: новый `labels.ts` переводит названия/описания всех параметров (включая строки тем «Линия N · Зелёная/Красная тема»), заголовки вкладок («История», «Журнал»), подписи и кнопки
- **Компактная компоновка**: уменьшены отступы и пустые места, KPI-карточки для ключевых чисел
- **Подсказки «?»**: у каждого параметра — кнопка «?» с попапом (назначение, диапазон, влияние, формула); длинные тексты под полями убраны
- **Параметры по категориям**: «Общие», «Математика краша», «Бустеры и линии», «Очки»
- **Редактор**: график модели — все параметры модели (α, мин/макс множитель, скорость роста, FPS, шаг кадра) слева, сам график справа; live-просмотр вероятностей P(X ≥ 2/5/10)
- **Без черновиков**: вместо «черновик → активация» одно действие «Сохранить и применить» (draft+activate на бэкенде выполняются прозрачно); страница «История» показывает только применённые версии и «Восстановить»
- **Симуляция**: понятная компоновка, KPI-результаты (итог, средний результат, выигрышные игры, теория), график с пунктирной контрастной теоретической линией и эмпирической кривой
- **Обзор**: бейдж «игра работает/остановлена», ключевые цифры карточками, блок «Как устроена игра» для не-технических пользователей
