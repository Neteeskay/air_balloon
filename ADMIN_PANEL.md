...
`V306__admin_panel.sql` — создаёт таблицы:
...
- **JDBC Instant映射**: исправлена ошибка десериализации `Instant` в JSON-ответах (problem на раундах)
...
| Unit-тесты (Vitest) | `npm test` — 49/49 PASS (40 существующих + 9 admin) |
| Docker build frontend | `docker compose build frontend` — успех |
| Backend smoke через nginx | POST login → token; GET current → 200; POST create → 201; POST activate → 200; 409 conflict → currentVersion; 400 validation → fieldErrors |
